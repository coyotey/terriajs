...
const Scales = {
    ...
    calculate(size, domain, data, xPadding=0) {
        ...
        if (!defined(domain)) {
            ...
            for (const theseUnits in domain.y) {
               if (domain.y.hasOwnProperty(theseUnits)) {
                    ...
                    if (dataWithTheseUnits.length === 1 && defined(dataWithTheseUnits[0].yAxisMin)) {
                        thisYDomain[0] = dataWithTheseUnits[0].yAxisMin;
                    }
                    if (dataWithTheseUnits.length === 1 && defined(dataWithTheseUnits[0].yAxisMax)) {
                        thisYDomain[1] = dataWithTheseUnits[0].yAxisMax;
                    }
                }
        ...
        xScale.range([xPadding, size.width - xPadding]).domain(domain.x);
