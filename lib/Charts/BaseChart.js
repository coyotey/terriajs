...
const BaseChart = {
    ...
    render(element, state) {
        ...
        const scales = Scales.calculate(size, state.domain, state.data, this.getXpadding(data));
        ...
        const g = d3Select(element).selectAll('.line-chart').attr('transform', gTransform);
        ...
        this.renderNoData(renderConext);

        // Axes.
        if (defined(scales)) {
            this.renderAxis(renderConext);
        }
    },
    ...
    renderNoData(renderConext)
    {
        const {g, size, margin, hasData, element} = renderConext;
    ...
    renderAxis(renderConext)
    {
        const {g, allUnits, size, scales, state, t, hasData, data} = renderConext;
        ...
        const mainYScale = scales.y[allUnits[0] || Scales.unknownUnits];
        let y0 = Math.min(Math.max(mainYScale(0), 0), size.plotHeight);
